...
def room(n)
    ...
    1.step(n) do |i|
        ...
        if min > roomprice[i-13]+33000 then min = roomprice[i-13]+33000; s = 13 end
...
def coins(n)
    ...
    1.step(n) do |i|
        ...
        if i > 4 && min > coinnumber[i-5] + 1 then min = coinnumber[i-5] + 1; s = 5 end
        if i > 9 && min > coinnumber[i-10] + 1 then min = coinnumber[i-10] + 1; s = 10 end
        if i > 24 &&  min > coinnumber[i-25] + 1 then min = coinnumber[i-25] + 1; s = 25 end
        coinnumber[i] = min
        coinsel[i] = s
    end
    a = [coinnumber[n]]
...
def pathfree(a)
    1.step(a.length-1) do |i|
        1.step(a[i].length-1) do |j| a[i][j] = a[i-1][j] + a[i][j-1] + a[i-1][j-1] end
    end
...
def path(a)
    ...
    8.times do |i|
        if a[0][i] < 0 then break end
        a[0][i] = 1
    end
    6.times do |i|
        if a[i][0] < 0 then break end
        a[i][0] = 1
    end
    1.step(5) do |i|
        1.step(7) do |j|
            if a[i][j] < 0 then next end
            if a[i-1][j] > 0 then a[i][j] += a[i-1][j] end
            if a[i][j-1] > 0 then a[i][j] += a[i][j-1] end
            if a[i-1][j-1] > 0 then a[i][j] += a[i-1][j-1] end
        end
    end
    return a[5][7]
end
