...
def lcstrace(s, t)
    ...
    ns = ''; 1.step(s.length) do ns = ' '+ns end  # ns:文字列 s で対応する文字の番号を記録
    nt = ''; 1.step(t.length) do nt = ' '+nt end  # nt:文字列 t で対応する文字の番号を記録
    d = 0  # 対応する文字のカウンタ
...
def aln(s, t)
#    端っこを考えて−２していく
    a = Array.new(t.length+1) do |i| Array.new(s.length+1) do |j| -2*(i+j) end end
    1.step(t.length) do |i|
        1.step(s.length) do |j|
            val = -1
            if s[j-1] == t[i-1] then
            ...
            if a[i][j-1] - 2 > val then val = a[i][j-1] - 2 end
            if a[i-1][j] - 2 > val then val = a[i-1][j] - 2 end
            a[i][j] = val
        end
    end
    return a[t.length][s.length]
